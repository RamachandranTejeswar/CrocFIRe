...
# Bank2: bottom-left, pins facing up
set bank2X $floor_leftX
set bank2Y $floor_bottomY
placeInstance $bank2_sram0 $bank2X $bank2Y R0

# Bank3: bottom-right, pins facing up
set bank3X [expr $floor_rightX - $RamSize512x32_W]
placeInstance $bank3_sram0 $bank3X $bank2Y R0
...
utl::report "SRAM bank2 bbox: ($bank2X, $bank2Y) - ([expr {$bank2X + $RamSize512x32_W}], [expr {$bank2Y + $RamSize512x32_H}]) R0"
utl::report "SRAM bank3 bbox: ($bank3X, $bank2Y) - ([expr {$bank3X + $RamSize512x32_W}], [expr {$bank2Y + $RamSize512x32_H}]) R0"
